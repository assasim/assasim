...
if [ $STEP0 = 1 ]; then
	...
	if [ -f "$1" ]; then
		...
		select ne in "new" "edit"; do
			case $ne in
				new ) echo "#ifndef MODEL_INCLUDED_" > $1;
					  echo "#define MODEL_INCLUDED_" >> $1;
					  echo "" >> $1;
...
while [ $RETRY = 1 ] && [ $STEP1 = 1 ]; do
	...
	if [ $MODE = 0 ]; then
		...
		if [ $FIRST = 1 ]; then
			$EDITOR "${DIR}/${FILE}_step1/${FILE}" "${DIR}/${FILE}_step1/behaviors.cpp" 2>/dev/null >/dev/null &
		fi
